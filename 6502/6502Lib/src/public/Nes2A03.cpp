...
void Nes2A03::clock()
{
    ...
    if (clock_counter % 6 == 0)
    {
        ...
        // else
        {
            pulse1_osc.frequency = 1789773.0 / (16.0 * static_cast<double>(pulse1_seq.reload + 1));
        ...
        //  else
        {
            pulse2_osc.frequency = 1789773.0 / (16.0 * static_cast<double>(pulse2_seq.reload + 1));
        ...
        noise_seq.clock(noise_enable,
                        [](uint32_t &s) { s = (((s & 0x0001) ^ ((s & 0x0002) >> 1)) << 14) | ((s & 0x7FFF) >> 1); });

        if (noise_lc.counter > 0 && noise_seq.timer >= 8)
        {
            noise_output = static_cast<double>(noise_seq.output) * (static_cast<double>(noise_env.output - 1) / 16.0);
        }
...
double Nes2A03::GetOutputSample()
{
    ...
    {
        return ((1.0 * pulse1_output) - 0.8) * 0.1 + ((1.0 * pulse2_output) - 0.8) * 0.1 +
