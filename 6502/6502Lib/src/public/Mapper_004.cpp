...
bool Mapper_004::cpuMapWrite(uint16_t addr, uint32_t &mapped_addr,
  ...
  if (addr >= 0x8000 && addr <= 0x9FFF) {
    ...
    if (!(addr & 0x0001)) {
      ...
      // as the value written still counts banks in 1KB units but odd numbered
      ...
      // R6 and R7 will ignore the top two bits, as the MMC3 has only 6 PRG ROM
      // address lines. Some romhacks rely on an 8-bit extension of R6/7 for
      // oversized PRG-ROM, but this is deliberately not supported by many
      // emulators. See iNES Mapper 004 below.
